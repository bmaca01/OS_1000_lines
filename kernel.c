...
// Returns if there are requests or not being processed
...
void read_write_disk(void *buf, unsigned sector, int is_write) {
    ...
    // Construct request accd to spec
    blk_req->sector = sector;
    blk_req->type = is_write ? VIRTIO_BLK_T_OUT : VIRTIO_BLK_T_IN;
    if (is_write)
        memcpy(blk_req->data, buf, SECTOR_SIZE);

    // Construct virtqueue descriptors
    ...
    // virtio-blk: If non-zero value is returned, it's an error
    if (blk_req->status != 0) {
...
void fs_init(void) {
    // Load the disk into `disk` global
...
void user_entry(void) {
    //PANIC("not yet implemented hehe");
    __asm__ __volatile__(
        "csrw sepc, %[sepc]\n"
...
void handle_syscall(struct trap_frame *f) {
    switch (f->a3) {
        ...
        case SYS_EXIT:
            ...
            /** Note:
             * Building a 'practical' OS requires freeing
             * resources held by the exited process like 
             * page tables and allocated memory
             */
